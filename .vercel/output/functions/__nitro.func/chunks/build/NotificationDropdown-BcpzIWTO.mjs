import { v as vueExports, a as useNuxtApp, b as useAuthStore, s as serverRenderer_cjs_prodExports, d as _sfc_main$6 } from './server.mjs';
import { formatDistanceToNow } from 'date-fns';
import { _ as _export_sfc } from './_plugin-vue_export-helper-1tPrXgE0.mjs';

const useNotifications = () => {
  const notifications = vueExports.ref([]);
...
const _sfc_main = /* @__PURE__ */ vueExports.defineComponent({
  ...
  setup(__props) {
    ...
    const dropdownRef = vueExports.ref(null);
    const isOpen = vueExports.ref(false);
    const isLoadingMore = vueExports.ref(false);
    const isRefreshing = vueExports.ref(false);
    const isMarkingAllAsRead = vueExports.ref(false);
    const isInitializing = vueExports.ref(true);
    vueExports.ref(null);
    const notifications = vueExports.computed(() => notificationsStore.notifications.value);
    const unreadCount = vueExports.computed(() => notificationsStore.unreadCount.value);
    const isLoading = vueExports.computed(() => notificationsStore.isLoading.value);
    const error = vueExports.computed(() => notificationsStore.error.value);
    const hasMorePages = vueExports.computed(
      () => notificationsStore.pagination.value.has_more_pages
    );
    const bellButtonClasses = vueExports.computed(() => [
    ...
    const iconClasses = vueExports.computed(() => [
    ...
    const notificationTitle = vueExports.computed(() => {
    ...
    const remainingCount = vueExports.computed(() => {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${serverRenderer_cjs_prodExports.ssrRenderAttrs(vueExports.mergeProps({
        ...
        ref_key: "dropdownRef",
        ref: dropdownRef
      }, _attrs))} data-v-7f7606b5><button class="${serverRenderer_cjs_prodExports.ssrRenderClass(bellButtonClasses.value)}"${serverRenderer_cjs_prodExports.ssrRenderAttr("title", notificationTitle.value)}${serverRenderer_cjs_prodExports.ssrIncludeBooleanAttr(isInitializing.value) ? " disabled" : ""} data-v-7f7606b5>`);
      _push(serverRenderer_cjs_prodExports.ssrRenderComponent(_component_Icon, {
        name: "fa:bell",
        class: iconClasses.value
      }, null, _parent));
      if (unreadCount.value > 0) {
        _push(`<span class="${serverRenderer_cjs_prodExports.ssrRenderClass(badgeClasses.value)}" data-v-7f7606b5>${serverRenderer_cjs_prodExports.ssrInterpolate(formatUnreadCount(unreadCount.value))}</span>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</button>`);
      if (isOpen.value) {
        ...
        if (unreadCount.value > 0) {
          _push(`<span class="text-blue-600" data-v-7f7606b5> (${serverRenderer_cjs_prodExports.ssrInterpolate(unreadCount.value)} unread) </span>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</h3><div class="flex items-center space-x-2" data-v-7f7606b5><button${serverRenderer_cjs_prodExports.ssrIncludeBooleanAttr(isRefreshing.value) ? " disabled" : ""} class="p-1 text-gray-400 hover:text-gray-600 rounded transition-colors" title="Refresh notifications" data-v-7f7606b5>`);
        _push(serverRenderer_cjs_prodExports.ssrRenderComponent(_component_Icon, {
        ...
        if (notifications.value.length > 0 && unreadCount.value > 0) {
          _push(`<button${serverRenderer_cjs_prodExports.ssrIncludeBooleanAttr(isMarkingAllAsRead.value) ? " disabled" : ""} class="text-xs text-blue-500 hover:text-blue-700 disabled:text-gray-400 transition-colors" data-v-7f7606b5>${serverRenderer_cjs_prodExports.ssrInterpolate(isMarkingAllAsRead.value ? "Marking..." : "Mark all as read")}</button>`);
        } else {
          _push(`<!---->`);
        }
        ...
        if (isLoading.value) {
          ...
          serverRenderer_cjs_prodExports.ssrRenderList(notifications.value, (notification) => {
            _push(`<div class="${serverRenderer_cjs_prodExports.ssrRenderClass(getNotificationItemClasses(notification))}" role="button" tabindex="0" data-v-7f7606b5><div class="flex items-start space-x-3" data-v-7f7606b5><div class="flex-shrink-0 mt-1" data-v-7f7606b5><div class="${serverRenderer_cjs_prodExports.ssrRenderClass(getNotificationIconClasses(notification))}" data-v-7f7606b5>`);
            _push(serverRenderer_cjs_prodExports.ssrRenderComponent(_component_Icon, {
            ...
            if (notification.title) {
              _push(`<p class="text-sm font-medium text-gray-900 line-clamp-1" data-v-7f7606b5>${serverRenderer_cjs_prodExports.ssrInterpolate(notification.title)}</p>`);
            } else {
              _push(`<!---->`);
            }
            _push(`<p class="${serverRenderer_cjs_prodExports.ssrRenderClass([{ "mt-1": notification.title }, "text-sm text-gray-600 line-clamp-2"])}" data-v-7f7606b5>${serverRenderer_cjs_prodExports.ssrInterpolate(notification.content)}</p></div>`);
            if (!notification.read) {
              _push(`<div class="h-2 w-2 bg-blue-500 rounded-full flex-shrink-0" aria-label="Unread notification" data-v-7f7606b5></div>`);
            } else {
              _push(`<!---->`);
            }
            _push(`</div><div class="flex justify-between items-center mt-2" data-v-7f7606b5><p class="text-xs text-gray-400" data-v-7f7606b5>${serverRenderer_cjs_prodExports.ssrInterpolate(formatTimestamp(notification.created_at))}</p>`);
            if (notification.priority && notification.priority !== "low") {
              _push(`<span class="${serverRenderer_cjs_prodExports.ssrRenderClass([getPriorityClasses(notification.priority), "text-xs px-1.5 py-0.5 rounded-full"])}" data-v-7f7606b5>${serverRenderer_cjs_prodExports.ssrInterpolate(notification.priority)}</span>`);
            } else {
              _push(`<!---->`);
            }
        ...
        if (hasMorePages.value && !isLoading.value) {
          _push(`<div class="px-4 py-3 border-t border-gray-100 bg-gray-50" data-v-7f7606b5><button${serverRenderer_cjs_prodExports.ssrIncludeBooleanAttr(isLoadingMore.value) ? " disabled" : ""} class="w-full text-sm text-blue-500 hover:text-blue-700 disabled:text-gray-400 disabled:cursor-not-allowed transition-colors font-medium" data-v-7f7606b5>`);
          ...
          _push(` ${serverRenderer_cjs_prodExports.ssrInterpolate(isLoadingMore.value ? "Loading more..." : `Load more (${remainingCount.value} remaining)`)}</button></div>`);
        } else {
          _push(`<!---->`);
        }
        if (error.value && !isLoading.value) {
          ...
          _push(`<p class="text-xs" data-v-7f7606b5>${serverRenderer_cjs_prodExports.ssrInterpolate(error.value)}</p></div></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div>`);
...
//# sourceMappingURL=NotificationDropdown-BcpzIWTO.mjs.map

...
const useWebSocket = pinia_prodExports.defineStore("websocket", () => {
  ...
  const socketPresence = vueExports.ref(null);
  const isPresenceConnected = vueExports.ref(false);
  const isPresenceConnecting = vueExports.ref(false);
  const reconnectAttempts = vueExports.ref(0);
  const maxReconnectAttempts = 8;
  const baseReconnectInterval = 1e3;
  const maxReconnectInterval = 3e4;
  const connectionError = vueExports.ref(null);
  const messageQueue = vueExports.ref([]);
  const activeSubscriptions = vueExports.ref([]);
  const authCheckInProgress = vueExports.ref(false);
  const heartbeatInterval = vueExports.ref(null);
  const lastHeartbeatTime = vueExports.ref(0);
  const pendingPings = vueExports.ref(/* @__PURE__ */ new Set());
  const connectionQuality = vueExports.ref(
    "disconnected"
  );
  const lastMessageTime = vueExports.ref(0);
  const networkChangeHandler = vueExports.ref(null);
  const maxQueueSize = 50;
  const messageRetryCount = vueExports.ref(/* @__PURE__ */ new Map());
  const maxRetries = 3;
  useRouter();
  ...
  const healthCheckInterval = vueExports.ref(null);
  ...
  return {
    // Connection State
    isConnected: vueExports.computed(
      () => isMessagesConnected.value && isPresenceConnected.value
    ),
    isConnecting: vueExports.computed(
      () => isMessagesConnecting.value || isPresenceConnecting.value
    ),
    isMessagesConnected: vueExports.computed(() => isMessagesConnected.value),
    isPresenceConnected: vueExports.computed(() => isPresenceConnected.value),
    connectionError: vueExports.computed(() => connectionError.value),
    connectionQuality: vueExports.computed(() => connectionQuality.value),
    // Queue Management
    queueLength: vueExports.computed(() => messageQueue.value.length),
    hasQueuedMessages: vueExports.computed(() => messageQueue.value.length > 0),
    // Connection Health
    reconnectAttempts: vueExports.computed(() => reconnectAttempts.value),
    maxReconnectAttempts: vueExports.computed(() => maxReconnectAttempts),
    // Core Actions
    connect,
    disconnect,
    reconnect,
...
const _sfc_main = /* @__PURE__ */ vueExports.defineComponent({
  ...
  setup(__props) {
    const props = __props;
    const hasError = vueExports.ref(false);
    const isLoading = vueExports.ref(!!props.src);
    const sizeClasses = {
      sm: "h-8 w-8",
      md: "h-10 w-10",
      lg: "h-12 w-12",
    ...
    const optimizedSrc = vueExports.computed(() => {
      var _a, _b, _c, _d, _e;
    ...
    const showFallback = vueExports.computed(() => !optimizedSrc.value || hasError.value);
    vueExports.watch(
      () => props.src,
      (newSrc) => {
        if (newSrc) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (showFallback.value) {
        ...
        _push(`<img${serverRenderer_cjs_prodExports.ssrRenderAttr("src", optimizedSrc.value)}${serverRenderer_cjs_prodExports.ssrRenderAttr("alt", _ctx.alt)} class="h-full w-full object-cover transition-opacity duration-200 ease-in-out" style="${serverRenderer_cjs_prodExports.ssrRenderStyle({
          imageRendering: "auto",

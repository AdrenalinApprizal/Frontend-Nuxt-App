import { _ as __nuxt_component_0 } from './NotificationDropdown-cZhdRx1L.mjs';
import { defineComponent, mergeProps, ref, watch, computed, resolveComponent, unref, useSSRContext } from 'vue';
...
import { u as useRouter, b as useAuthStore, c as _imports_0, e as useRoute$1, a as useNuxtApp } from './server.mjs';
import 'date-fns';
...
const _sfc_main$2 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "p-3 bg-white rounded-md border border-gray-200 shadow-sm flex items-center gap-3" }, _attrs))}><div class="h-10 w-10 rounded-full overflow-hidden bg-gray-200 flex-shrink-0 flex items-center justify-center">`);
      if (unref(userAvatar)) {
        _push(`<img${ssrRenderAttr("src", unref(userAvatar))}${ssrRenderAttr("alt", unref(userName))} class="h-full w-full object-cover">`);
      } else {
      ...
      _push(`</div><div class="flex-grow"><h4 class="font-medium text-sm text-gray-800">${ssrInterpolate(unref(userName))}</h4><p class="text-xs text-gray-500">Wants to be your friend</p></div><div class="flex space-x-2"><button class="p-2 bg-gray-100 rounded-full hover:bg-gray-200 transition-colors" aria-label="Reject">`);
      _push(ssrRenderComponent(_component_Icon, {
        name: "fa:times",
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    computed(() => friendsStore.pendingRequests);
    ...
    return (_ctx, _push, _parent, _attrs) => {
      const _component_NotificationDropdown = __nuxt_component_0;
      const _component_Icon = resolveComponent("Icon");
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "h-full flex flex-col p-6 bg-white" }, _attrs))}><div class="mb-6 flex justify-between items-center"><h1 class="text-xl font-bold text-gray-800">Friends</h1><div class="flex items-center space-x-2"><div class="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center">`);
      ...
      _push(`</div><input type="text"${ssrRenderAttr("value", unref(searchQuery))} placeholder="Search friends" class="pl-11 w-full p-3 bg-gray-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-300 transition-all text-sm">`);
      if (unref(isSearchingFriends)) {
      ...
      _push(`</div>`);
      if (unref(friendsStore).isLoading) {
        ...
        if (unref(incomingRequests).length > 0) {
          ...
          _push(` Friend Requests <span class="ml-2 px-1.5 py-0.5 bg-blue-500 text-white text-xs rounded-full">${ssrInterpolate(unref(incomingRequests).length)}</span></h2><button class="text-blue-700 hover:text-blue-900 p-1">`);
          _push(ssrRenderComponent(_component_Icon, {
            name: unref(requestsHidden) ? "lucide:chevron-down" : "lucide:chevron-up",
            class: "h-4 w-4"
          }, null, _parent));
          _push(`</button></div>`);
          if (!unref(requestsHidden)) {
            _push(`<div class="p-3 space-y-2 bg-blue-50"><!--[-->`);
            ssrRenderList(unref(incomingRequests), (request) => {
        ...
        _push(`<div class="flex-1 overflow-auto"><h2 class="font-medium text-gray-500 text-xs uppercase tracking-wider mb-3">${ssrInterpolate(unref(onlineFriendsCount) > 0 ? "Online" : "All")} Friends `);
        if (unref(sortedFriends).length > 0) {
          _push(`<span class="ml-2 text-gray-400"> (${ssrInterpolate(unref(sortedFriends).length)}) </span>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</h2>`);
        if (unref(sortedFriends).length === 0) {
          ...
          _push(`<p class="text-gray-500 font-medium">${ssrInterpolate(unref(searchQuery) ? `No results for "${unref(searchQuery)}"` : "No friends yet")}</p><p class="text-sm text-gray-400 mt-2"> Add friends to start chatting </p><button class="mt-4 px-4 py-2 bg-blue-500 text-white rounded-md flex items-center text-sm">`);
          _push(ssrRenderComponent(_component_Icon, {
            name: "lucide:user-plus",
            class: "mr-2 h-3.5 w-3.5"
          }, null, _parent));
          ...
          ssrRenderList(unref(sortedFriends), (friend) => {
            _push(`<div class="${ssrRenderClass(`flex items-center p-3 rounded-xl transition-all cursor-pointer ${_ctx.$route.path === `/chat/messages/${friend.id}` ? "bg-blue-50 border border-blue-200 shadow-sm" : "hover:bg-gray-50 border border-transparent"}`)}"><div class="relative mr-3"><div class="h-11 w-11 rounded-full overflow-hidden bg-gray-200 flex-shrink-0 flex items-center justify-center">`);
            if (friend.profile_picture_url || friend.avatar) {
              _push(`<img${ssrRenderAttr("src", friend.profile_picture_url || friend.avatar)}${ssrRenderAttr("alt", friend.name)} class="h-full w-full object-cover">`);
            } else {
              _push(ssrRenderComponent(_component_Icon, {
                name: "fa:user",
                class: "h-5 w-5 text-gray-500"
            ...
            _push(`</div><div class="flex-1 min-w-0"><div class="flex justify-between items-center"><h3 class="font-semibold text-gray-900 truncate">${ssrInterpolate(getFriendDisplayName(friend))}</h3><span class="text-xs text-gray-500 whitespace-nowrap ml-2">${ssrInterpolate(friend.last_active ? formatLastActive(friend.last_active) : formatLastActive(getLastActive(friend.id)))}</span></div><div class="flex items-center mt-0.5"><p class="text-xs text-gray-500 truncate flex-1">${ssrInterpolate(getFriendStatus(friend.id) === "online" ? "Active now" : friend.username ? `@${friend.username}` : friend.email || "offline")}</p>`);
            if (friend.unread_count && friend.unread_count > 0) {
              _push(`<span class="inline-flex items-center justify-center px-2 py-0.5 ml-2 text-xs font-medium leading-none text-white bg-red-500 rounded-full">${ssrInterpolate(friend.unread_count)}</span>`);
            } else {
              _push(`<!---->`);
            }
      ...
      if (unref(showAddFriendPopup)) {
        ...
        _push(`</button></div><p class="text-sm text-gray-600 mb-4"> You can add a friend with their username. It&#39;s case sensitive! </p><form class="space-y-4"><div><label class="block text-sm font-medium text-gray-700 mb-2"> Username </label><div class="relative"><input type="text"${ssrRenderAttr("value", unref(addByUsername))} placeholder="Enter friend&#39;s username" class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all pl-10 text-gray-900 placeholder-gray-500"${ssrIncludeBooleanAttr(unref(isAddingByUsername)) ? " disabled" : ""}><div class="absolute left-3 top-1/2 transform -translate-y-1/2">`);
        _push(ssrRenderComponent(_component_Icon, {
          name: "fa:user",
          class: "h-4 w-4 text-gray-400"
        }, null, _parent));
        _push(`</div></div></div><div class="flex justify-end space-x-3 pt-2"><button type="button" class="px-4 py-2.5 text-gray-700 hover:bg-gray-100 rounded-lg transition-colors"> Cancel </button><button type="submit" class="px-5 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:bg-blue-300 flex items-center"${ssrIncludeBooleanAttr(!unref(addByUsername).trim() || unref(isAddingByUsername)) ? " disabled" : ""}>`);
        if (unref(isAddingByUsername)) {
        ...
        _push(` ${ssrInterpolate(unref(isAddingByUsername) ? "Sending..." : "Send Request")}</button></div></form></div></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div>`);
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(_component_ChatFriendsList, null, null, _parent));
...
//# sourceMappingURL=friends-NdX0cgrL.mjs.map

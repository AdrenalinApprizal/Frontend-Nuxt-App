import process from 'node:process';globalThis._importMeta_=globalThis._importMeta_||{url:"file:///_entry.js",env:process.env};import { w as withLeadingSlash, j as joinRelativeURL, u as useRuntimeConfig, f as getResponseStatusText, h as getResponseStatus, i as defineRenderHandler, c as getQuery, k as createError, l as getRouteRules, m as useNitroApp } from '../nitro/nitro.mjs';
import { g as getDefaultExportFromNamespaceIfNotNamed, s as shared_cjs_prod } from '../_/shared.cjs.prod.mjs';
import * as decode from 'entities/lib/decode.js';
...
const require$$1$1 = /*@__PURE__*/getDefaultExportFromNamespaceIfNotNamed(decode);
...
const require$$3$1 = /*@__PURE__*/getDefaultExportFromNamespaceIfNotNamed(estreeWalker$1);
...
var decode_js = require$$1$1;
var parser = lib;
var estreeWalker = require$$3$1;
...
function createObjectProperty(key, value) {
  return {
    type: 16,
    loc: locStub,
    key: shared$2.isString(key) ? createSimpleExpression(key, true) : key,
    value
  };
}
...
function createInterpolation(content, loc) {
  return {
    type: 5,
    loc,
    content: shared$2.isString(content) ? createSimpleExpression(content, false, loc) : content
  };
}
...
function findDir(node, name, allowEmpty = false) {
  for (let i = 0; i < node.props.length; i++) {
    ...
    if (p.type === 7 && (allowEmpty || p.exp) && (shared$2.isString(name) ? p.name === name : name.test(p.name))) {
      return p;
    }
  }
}
...
function getUnnormalizedProps(props, callPath = []) {
  if (props && !shared$2.isString(props) && props.type === 14) {
    const callee = props.callee;
    if (!shared$2.isString(callee) && propsHelperSet.has(callee)) {
...
function injectProp(node, prop, context) {
  ...
  if (props == null || shared$2.isString(props)) {
    propsWithInjection = createObjectExpression([prop]);
  } else if (props.type === 14) {
    const first = props.arguments[0];
    if (!shared$2.isString(first) && first.type === 15) {
...
function hasScopeRef(node, ids) {
  ...
  switch (node.type) {
    ...
    case 8:
      return node.children.some((c) => shared$2.isObject(c) && hasScopeRef(c, ids));
    case 5:
    case 12:
...
const defaultParserOptions = {
  ...
  delimiters: [`{{`, `}}`],
  getNamespace: () => 0,
  isVoidTag: shared$2.NO,
  isPreTag: shared$2.NO,
  isIgnoreNewlineTag: shared$2.NO,
  isCustomElement: shared$2.NO,
...
function onCloseTag(el, end, isImplied = false) {
  ...
  if (tokenizer.inSFCRoot) {
    if (el.children.length) {
      el.innerLoc.end = shared$2.extend({}, el.children[el.children.length - 1].loc.end);
    } else {
      el.innerLoc.end = shared$2.extend({}, el.innerLoc.start);
    }
...
function baseParse(input, options) {
  reset();
  currentInput = input;
  currentOptions = shared$2.extend({}, defaultParserOptions);
  if (options) {
    let key;
    for (key in options) {
      if (options[key] != null) {
...
function walk(node, parent, context, doNotHoistNode = false, inFor = false) {
  ...
  if (toCache.length === children.length && node.type === 1) {
    if (node.tagType === 0 && node.codegenNode && node.codegenNode.type === 13 && shared$2.isArray(node.codegenNode.children)) {
      node.codegenNode.children = getCacheExpression(
        createArrayExpression(node.codegenNode.children)
      );
      cachedAsArray = true;
    } else if (node.tagType === 1 && node.codegenNode && node.codegenNode.type === 13 && node.codegenNode.children && !shared$2.isArray(node.codegenNode.children) && node.codegenNode.children.type === 15) {
    ...
    } else if (node.tagType === 3 && parent && parent.type === 1 && parent.tagType === 1 && parent.codegenNode && parent.codegenNode.type === 13 && parent.codegenNode.children && !shared$2.isArray(parent.codegenNode.children) && parent.codegenNode.children.type === 15) {
  ...
  if (slotCacheKeys.length && node.type === 1 && node.tagType === 1 && node.codegenNode && node.codegenNode.type === 13 && node.codegenNode.children && !shared$2.isArray(node.codegenNode.children) && node.codegenNode.children.type === 15) {
  ...
  function getSlotNode(node2, name) {
    if (node2.children && !shared$2.isArray(node2.children) && node2.children.type === 15) {
  ...
  switch (node.type) {
    ...
    case 8:
      ...
      for (let i = 0; i < node.children.length; i++) {
        ...
        if (shared$2.isString(child) || shared$2.isSymbol(child)) {
          continue;
        }
...
function getConstantTypeOfHelperCall(value, context) {
  if (value.type === 14 && !shared$2.isString(value.callee) && allowHoistedHelperSet.has(value.callee)) {
...
function createTransformContext(root, {
  ...
  directiveTransforms = {},
  transformHoist = null,
  isBuiltInComponent = shared$2.NOOP,
  isCustomElement = shared$2.NOOP,
  expressionPlugins = [],
  scopeId = null,
  slotted = true,
  ssr = false,
  inSSR = false,
  ssrCssVars = ``,
  bindingMetadata = shared$2.EMPTY_OBJ,
  inline = false,
  isTS = false,
  onError = defaultOnError,
  onWarn = defaultOnWarn,
  ...
  const context = {
    // options
    filename,
    selfName: nameMatch && shared$2.capitalize(shared$2.camelize(nameMatch[1])),
    prefixIdentifiers,
    hoistStatic,
    hmr,
    cacheHandlers,
    ...
    onNodeRemoved: shared$2.NOOP,
    addIdentifiers(exp) {
      {
        if (shared$2.isString(exp)) {
          addId(exp);
    ...
    hoist(exp) {
      if (shared$2.isString(exp)) exp = createSimpleExpression(exp);
...
function traverseChildren(parent, context) {
  ...
  for (; i < parent.children.length; i++) {
    ...
    if (shared$2.isString(child)) continue;
...
function createStructuralDirectiveTransform(name, fn) {
  const matches = shared$2.isString(name) ? (n) => n === name : (n) => name.test(n);
  return (node, context) => {
    if (node.type === 1) {
      const { props } = node;
...
function isText(n) {
  return shared$2.isString(n) || n.type === 4 || n.type === 2 || n.type === 5 || n.type === 8;
}
function genNodeListAsArray(nodes, context) {
  const multilines = nodes.length > 3 || nodes.some((n) => shared$2.isArray(n) || !isText(n));
  context.push(`[`);
...
function genNodeList(nodes, context, multilines = false, comma = true) {
  ...
  for (let i = 0; i < nodes.length; i++) {
    const node = nodes[i];
    if (shared$2.isString(node)) {
      push(node, -3 /* Unknown */);
    } else if (shared$2.isArray(node)) {
...
function genCallExpression(node, context) {
  const { push, helper, pure } = context;
  const callee = shared$2.isString(node.callee) ? node.callee : helper(node.callee);
  if (pure) {
    push(PURE_ANNOTATION);
  }
...
const isLiteralWhitelisted = /* @__PURE__ */ shared$2.makeMap("true,false,null,this");
...
function processExpression(node, context, asParams = false, asRawStatements = false, localVars = Object.create(context.identifiers)) {
  ...
  const rewriteIdentifier = (raw, parent, id) => {
    const type = shared$2.hasOwn(bindingMetadata, raw) && bindingMetadata[raw];
    if (inline) {
...
` : ``} ? ${prefix}${raw}.value${postfix} : ${prefix}${raw}${postfix}`;
        ...
        return shared$2.genPropsAccessExp(raw);
      } else if (type === "props-aliased") {
        return shared$2.genPropsAccessExp(bindingMetadata.__propsAliases[raw]);
      }
    } else {
  ...
  if (ast === null || !ast && isSimpleIdentifier(rawExp)) {
    ...
    const isAllowedGlobal = shared$2.isGloballyAllowed(rawExp);
...
function canPrefix(id) {
  if (shared$2.isGloballyAllowed(id.name)) {
    return false;
  }
  if (id.name === "require") {
    return false;
...
const transformBind = (dir, _node, context) => {
  ...
  if (modifiers.some((mod) => mod.content === "camel")) {
    if (arg.type === 4) {
      if (arg.isStatic) {
        arg.content = shared$2.camelize(arg.content);
      } else {
...
const transformBindShorthand = (dir, context) => {
  const arg = dir.arg;
  const propName = shared$2.camelize(arg.content);
...
const transformElement = (node, context) => {
  return function postTransformElement() {
    ...
    const isDynamicComponent = shared$2.isObject(vnodeTag) && vnodeTag.callee === RESOLVE_DYNAMIC_COMPONENT;
    let vnodeProps;
    let vnodeChildren;
    let patchFlag = 0;
...
function resolveComponentType(node, context, ssr = false) {
  ...
  if (context.selfName && shared$2.capitalize(shared$2.camelize(tag)) === context.selfName) {
...
function resolveSetupReference(name, context) {
  ...
  const camelName = shared$2.camelize(name);
  const PascalName = shared$2.capitalize(camelName);
...
function buildProps(node, context, props = node.props, isComponent, isDynamicComponent, ssr = false) {
  ...
  const analyzePatchFlag = ({ key, value }) => {
    if (isStaticExp(key)) {
      ...
      const isEventHandler = shared$2.isOn(name);
      if (isEventHandler && (!isComponent || isDynamicComponent) && // omit the flag for click handlers because hydration gives click
      // dedicated fast path.
      name.toLowerCase() !== "onclick" && // omit v-model handlers
      name !== "onUpdate:modelValue" && // omit onVnodeXXX hooks
      !shared$2.isReservedProp(name)) {
        hasHydrationEventBinding = true;
      }
      if (isEventHandler && shared$2.isReservedProp(name)) {
        hasVnodeHook = true;
      }
    ...
    if (prop.type === 6) {
      ...
      if (directiveTransform) {
        ...
        if (needRuntime) {
          ...
          if (shared$2.isSymbol(needRuntime)) {
            directiveImportMap.set(prop, needRuntime);
          }
        }
      } else if (!shared$2.isBuiltInDirective(name)) {
        runtimeDirectives.push(prop);
        if (hasChildren) {
          shouldUseBlock = true;
        }
...
function dedupeProperties(properties) {
  ...
  for (let i = 0; i < properties.length; i++) {
    ...
    if (existing) {
      if (name === "style" || name === "class" || shared$2.isOn(name)) {
    ...
    if (p.type === 6) {
      if (p.value) {
        if (p.name === "name") {
          ...
          p.name = shared$2.camelize(p.name);
          nonNameProps.push(p);
        }
      }
    } else {
      if (p.name === "bind" && isStaticArgOf(p.arg, "name")) {
        if (p.exp) {
          ...
          const name = shared$2.camelize(p.arg.content);
        ...
        if (p.name === "bind" && p.arg && isStaticExp(p.arg)) {
          p.arg.content = shared$2.camelize(p.arg.content);
        }
        nonNameProps.push(p);
      }
    }
  ...
  if (arg.type === 4) {
    if (arg.isStatic) {
      ...
      const eventString = node.tagType !== 0 || rawName.startsWith("vnode") || !/[A-Z]/.test(rawName) ? (
        ...
        shared$2.toHandlerKey(shared$2.camelize(rawName))
      ) : (
...
const transformModel = (dir, node, context) => {
  ...
  const eventName = arg ? isStaticExp(arg) ? `onUpdate:${shared$2.camelize(arg.content)}` : createCompoundExpression(['"onUpdate:" + ', arg]) : `onUpdate:modelValue`;
  let assignmentExp;
...
function baseCompile(source, options = {}) {
  ...
  const resolvedOptions = shared$2.extend({}, options, {
    prefixIdentifiers
  });
  const ast = shared$2.isString(source) ? baseParse(source, resolvedOptions) : source;
  ...
  transform(
    ast,
    shared$2.extend({}, resolvedOptions, {
      nodeTransforms: [
        ...nodeTransforms,
        ...options.nodeTransforms || []
        // user transforms
      ],
      directiveTransforms: shared$2.extend(
        {},
        directiveTransforms,
...
compilerCore_cjs_prod.generateCodeFrame = shared$2.generateCodeFrame;
...
const appHead = {"meta":[{"name":"viewport","content":"width=device-width, initial-scale=1"},{"charset":"utf-8"}],"link":[],"style":[],"script":[],"noscript":[],"htmlAttrs":{"class":"antialiased"},"bodyAttrs":{"class":"bg-white dark:bg-black dark:text-white font-sans"}};
...
export { buildAssetsURL as a, baseURL as b, headSymbol as h, lib as l, publicAssetsURL as p, renderer$1 as r, useHead as u };
